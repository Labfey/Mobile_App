...
// IMPORT ALL DATABASE FUNCTIONS FROM THE LIBRARY
import { ref, onValue, update, remove, get, push } from "firebase/database";
// ONLY IMPORT YOUR CONFIGURATION FROM YOUR SERVICE FILE
...
export default function MapsScreen() {
  ...
  const [isFareVisible, setIsFareVisible] = useState(false);
  ...
  const mapHtml = `
    ...
    <head>
      ...
      <style>
        ...
        .jeep-marker { width: 32px; height: 32px; background: #15803D; border: 2px solid white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; }
    ...
    <body>
      ...
      <script>
        ...
        window.addEventListener("message", (event) => {
          ...
          if (m.type === "CLEAR_ZONES") { segments.forEach(s => map.removeLayer(s)); segments = []; }
          if (m.type === "SHIFT_ZONE") { if (segments.length > 0) { map.removeLayer(segments.shift()); } }
          if (m.type === "DRAW_ZONES") {
            ...
            m.zones.forEach((z) => {
              ...
              poly.on('click', () => window.ReactNativeWebView.postMessage(JSON.stringify({type: 'OPEN_FARE', originalIndex: z.originalIndex})));
              segments.push(poly);
            });
          }
          if (m.type === "SET_JEEPS") {
            m.jeeps.forEach(j => {
              if (jeepMarkers[j.id]) {
                ...
                jeepMarkers[j.id] = L.marker([j.lat, j.lng], {icon: L.divIcon({className: 'jeep-marker', iconSize: [32, 32], html: '🚕'})}).addTo(map);
              }
            });
          }
        });
  ...
  const drawSegmentedRoute = async (destination: 'Town' | 'Balacbac') => {
    ...
    const zoneData = await Promise.all(zonesToProcess.map(async (zone) => {
      ...
      try {
        ...
        return { coords: data.routes[0].geometry.coordinates.map((c: any) => [c[1], c[0]]), color: zone.color, originalIndex: zone.originalIndex };
      } catch (e) { return { coords: [], color: zone.color, originalIndex: zone.originalIndex }; }
    }));
  ...
  const endTrip = () => {
    try {
        if (auth.currentUser && tripStartTime) {
            const uid = auth.currentUser.uid;
            const now = new Date();
            const dateKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
            
            const safeDistance = isNaN(tripDistance) || tripDistance < 0 ? 0 : tripDistance;
            
            push(ref(db, `history/${uid}/${dateKey}`), {
                route: currentDest === 'Town' ? "To Town" : "To Balacbac",
                startTime: new Date(tripStartTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                endTime: now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                distance: parseFloat(safeDistance.toFixed(2)),
                timestamp: Date.now()
            });
        }
        ...
        webViewRef.current?.postMessage(JSON.stringify({ type: "CLEAR_ZONES" }));
        Alert.alert("Success", "Trip saved to Dashboard.");
    } catch (e) { Alert.alert("Error", "Could not save trip."); }
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      locationSub.current = await Location.watchPositionAsync({ accuracy: 3, distanceInterval: 5 }, (pos) => {
        ...
        if (isDriverOnline && lastCoordRef.current) {
            const d = getDistance(lastCoordRef.current.lat, lastCoordRef.current.lng, latitude, longitude);
            setTripDistance(p => p + (d / 1000));
        }
        lastCoordRef.current = { lat: latitude, lng: longitude };
        
        if (activeZonesRef.current.length > 0) {
            const z = activeZonesRef.current[0];
            if (z.targetNode && getDistance(latitude, longitude, z.targetNode[0], z.targetNode[1]) < 100) {
  ...
  return (
    <View style={s.container}>
      <WebView ref={webViewRef} source={{ html: mapHtml }} onMessage={(e) => {
          ...
          if (d.type === 'OPEN_FARE' && role !== 'driver') { setSelectedFareIndex(d.originalIndex); setIsFareVisible(true); }
      }} style={s.map} />
      ...
      <Modal animationType="slide" transparent visible={isRouteModalVisible}>
        <View style={s.modalOverlay}><View style={s.modalContent}>
            ...
            <TouchableOpacity style={[s.routeChoice, {backgroundColor: '#15803D'}]} onPress={() => { setIsRouteModalVisible(false); setIsDriverOnline(true); setCurrentDest('Town'); setTripStartTime(Date.now()); drawSegmentedRoute('Town'); }}><Text style={s.routeChoiceText}>To Town</Text></TouchableOpacity>
            <TouchableOpacity style={[s.routeChoice, {backgroundColor: '#1D4ED8'}]} onPress={() => { setIsRouteModalVisible(false); setIsDriverOnline(true); setCurrentDest('Balacbac'); setTripStartTime(Date.now()); drawSegmentedRoute('Balacbac'); }}><Text style={s.routeChoiceText}>To Balacbac</Text></TouchableOpacity>
            <TouchableOpacity onPress={() => setIsRouteModalVisible(false)}><Text style={s.cancelText}>Cancel</Text></TouchableOpacity>
        </View></View>
      </Modal>
      ...
      {role === 'driver' && (
        <View style={s.driverPanel}>
          {isDriverOnline && (
            <View style={s.subRow}>
              <TouchableOpacity onPress={() => currentDest && drawSegmentedRoute(currentDest)} style={s.subBtn}><RefreshCw color="white" size={16} /><Text style={s.subBtnText}> RE-ROUTE</Text></TouchableOpacity>
              <TouchableOpacity onPress={() => setIsJeepFull(!isJeepFull)} style={[s.subBtn, isJeepFull ? s.btnRed : s.btnGray]}><Users color="white" size={16} /><Text style={s.subBtnText}>{isJeepFull ? " FULL" : " AVAILABLE"}</Text></TouchableOpacity>
            </View>
          )}
          <TouchableOpacity style={[s.mainBtn, isDriverOnline ? s.btnRed : s.btnGreen]} onPress={() => isDriverOnline ? endTrip() : setIsRouteModalVisible(true)}>
            <NavIcon color="white" size={18} /><Text style={s.mainBtnText}>{isDriverOnline ? " END TRIP" : " START DRIVING"}</Text>
          </TouchableOpacity>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  modalContent: { width: '80%', backgroundColor: 'white', padding: 25, borderRadius: 25, alignItems: 'center' },
  modalTitle: { fontSize: 18, fontWeight: 'bold', marginBottom: 20 },
  routeChoice: { width: '100%', padding: 16, borderRadius: 15, marginBottom: 12, alignItems: 'center' },
  routeChoiceText: { color: 'white', fontWeight: 'bold' },
  cancelText: { color: '#9CA3AF', marginTop: 10, fontWeight: 'bold' },
  ...
  mainBtn: { padding: 18, borderRadius: 20, alignItems: 'center', elevation: 5, flexDirection: 'row', justifyContent: 'center' },
