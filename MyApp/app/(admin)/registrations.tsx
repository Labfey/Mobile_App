...
import { CheckCircle, XCircle, Eye, FileText, Clock, User, CreditCard, X, UserPlus, Lock, Mail } from "lucide-react-native";
...
interface Registration {
    ...
    licenseNumber: string; licenseImage: string; idImage: string;
    status: RegStatus; submittedAt: number;
}
...
// ── Create driver account modal (admin-initiated) ─────────────────────────────
function CreateDriverModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
    ...
    const handleCreate = async () => {
        if (!name.trim() || !email.trim() || !password || !plate.trim()) {
        ...
        try {
            ...
            // Write user record
            await set(ref(db, `users/${uid}`), { username: name, email, role: "driver", createdAt: Date.now(), createdByAdmin: true });
            // Write jeep_info stub
            await set(ref(db, `jeep_info/${uid}`), { driverName: name, plate: plate.toUpperCase(), route: "Balacbac To Town", updatedAt: Date.now() });
            Alert.alert("✅ Driver Created", `${name} has been registered as a driver and can now log in.`);
            setName(""); setEmail(""); setPassword(""); setPlate("");
            onClose();
        } catch (err: any) {
            ...
            <View style={cs.overlay}>
                <View style={cs.sheet}>
                    ...
                    <View style={cs.hdr}>
                        ...
                        <View style={{ flex: 1 }}><Text style={cs.hdrTitle}>Create Driver Account</Text><Text style={cs.hdrSub}>Admin-created — instant access</Text></View>
...
const cs = StyleSheet.create({
    ...
    hdrTitle: { fontSize: 17, fontWeight: "800", color: "#111827" }, hdrSub: { fontSize: 12, color: "#6b7280", marginTop: 2 },
...
export default function AdminRegistrations() {
    ...
    useEffect(() => {
        const unsub = onValue(ref(db, "pending_registrations"), (snap) => {
            ...
            setRegistrations(Object.entries(data).map(([uid, v]) => ({ uid, ...v } as Registration)).sort((a, b) => b.submittedAt - a.submittedAt));
            setLoading(false);
        });
        return () => unsub();
    }, []);
    ...
    const handleApprove = (reg: Registration) => {
        Alert.alert("Approve Driver", `Confirm approval for ${reg.fullName}?`, [
            ...
            { text: "Approve", onPress: async () => {
                ...
                try {
                    await update(ref(db, `users/${reg.uid}`), { role: "driver" });
                    await update(ref(db, `pending_registrations/${reg.uid}`), { status: "approved", reviewedAt: Date.now() });
                    setSelected(null);
        ...
        <SafeAreaView style={s.container}>
            ...
            <View style={s.statRow}>
                {(["pending", "approved", "rejected"] as RegStatus[]).map(status => {
                    ...
                    return (
                        <TouchableOpacity key={status} style={[s.statCard, { backgroundColor: cfg.bg }, activeFilter === status && s.statCardActive]} onPress={() => setActiveFilter(activeFilter === status ? "all" : status)}>
                            <View style={[s.statDot, { backgroundColor: cfg.dot }]} />
                            <Text style={[s.statCount, { color: cfg.color }]}>{counts[status]}</Text>
                            <Text style={[s.statLabel, { color: cfg.color }]}>{status.charAt(0).toUpperCase() + status.slice(1)}</Text>
                        </TouchableOpacity>
                    );
                })}
            </View>
            ...
            {loading ? <ActivityIndicator color="#15803d" style={{ marginTop: 40 }} /> : (
                <ScrollView contentContainerStyle={s.list}>
                    {filtered.length === 0 ? (
                        <View style={s.emptyBox}><Text style={{ fontSize: 40, marginBottom: 12 }}>📋</Text><Text style={s.emptyText}>No {activeFilter === "all" ? "" : activeFilter} applications.</Text></View>
                    ) : filtered.map(reg => {
                        const cfg = STATUS_CFG[reg.status];
                        return (
                            <TouchableOpacity key={reg.uid} style={s.card} onPress={() => setSelected(reg)} activeOpacity={0.75}>
                                <View style={s.avatar}><Text style={s.avatarText}>{reg.fullName?.charAt(0)?.toUpperCase() ?? "D"}</Text></View>
                                <View style={s.cardInfo}>
                                    <Text style={s.cardName}>{reg.fullName}</Text>
                                    <Text style={s.cardEmail}>{reg.email}</Text>
                                    <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 4 }}><Clock size={10} color="#9ca3af" /><Text style={{ fontSize: 10, color: "#9ca3af" }}>{timeAgo(reg.submittedAt)}</Text></View>
                                </View>
            ...
            <Modal visible={!!selected} animationType="slide" transparent>
                <View style={s.modalOverlay}>
                    <View style={[s.modalSheet, { height: height * 0.88 }]}>
                        <View style={s.sheetHandle} />
                        {selected && <>
                            <View style={s.mHdr}>
                                <View style={s.mAvatar}><Text style={s.mAvatarTxt}>{selected.fullName?.charAt(0)?.toUpperCase()}</Text></View>
                                <View style={{ flex: 1 }}><Text style={s.mName}>{selected.fullName}</Text><Text style={s.mEmail}>{selected.email}</Text></View>
                                <TouchableOpacity onPress={() => setSelected(null)} style={s.closeBtn}><X color="#6b7280" size={20} /></TouchableOpacity>
                            </View>
                            <ScrollView showsVerticalScrollIndicator={false}>
                                {/* Info rows */}
                                <View style={s.infoRow}><View style={[s.infoIcon, { backgroundColor: "#dbeafe" }]}><CreditCard color="#3b82f6" size={18} /></View><View><Text style={s.infoLbl}>License Number</Text><Text style={s.infoVal}>{selected.licenseNumber}</Text></View></View>
                                <View style={s.infoRow}><View style={[s.infoIcon, { backgroundColor: "#f0fdf4" }]}><Clock color="#15803d" size={18} /></View><View><Text style={s.infoLbl}>Submitted</Text><Text style={s.infoVal}>{new Date(selected.submittedAt).toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" })}</Text></View></View>
                                {/* Documents */}
                                <Text style={s.docSec}>Submitted Documents</Text>
                                <View style={s.docsRow}>
                                    {[{ uri: selected.licenseImage, label: "Driver's License" }, { uri: selected.idImage, label: "Government ID" }].map((doc, i) => (
                                        <TouchableOpacity key={i} style={s.docBox} onPress={() => setImagePreview(doc.uri)}>
                                            <Image source={{ uri: doc.uri }} style={s.docThumb} />
                                            <View style={s.docLbl}><FileText color="#3b82f6" size={12} /><Text style={s.docLblTxt}>{doc.label}</Text></View>
                                ...
                                {selected.status === "pending" && (
                                    <View style={s.actionRow}>
                                        <TouchableOpacity style={s.rejectBtn} onPress={() => handleReject(selected)} disabled={!!processing}>
                                            <XCircle color="#dc2626" size={20} /><Text style={s.rejectTxt}>Reject</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={s.approveBtn} onPress={() => handleApprove(selected)} disabled={!!processing}>
                                            {processing === selected.uid ? <ActivityIndicator color="white" /> : <><CheckCircle color="white" size={20} /><Text style={s.approveTxt}>Approve</Text></>}
                                        </TouchableOpacity>
                                    </View>
                                )}
                                {selected.status !== "pending" && (
                                    <View style={[s.statusResult, { backgroundColor: STATUS_CFG[selected.status].bg }]}>
                                        {selected.status === "approved" ? <CheckCircle color="#15803d" size={20} /> : <XCircle color="#dc2626" size={20} />}
                                        <Text style={[s.statusResultTxt, { color: STATUS_CFG[selected.status].color }]}>This application has been {selected.status}.</Text>
                                    </View>
                                )}
                            </ScrollView>
                        </>}
            ...
            <Modal visible={!!imagePreview} transparent animationType="fade">
                <View style={s.imgOverlay}>
                    ...
                    <TouchableOpacity style={s.imgClose} onPress={() => setImagePreview(null)}><X color="white" size={24} /></TouchableOpacity>
                </View>
            </Modal>
...
const s = StyleSheet.create({
    ...
    title: { fontSize: 22, fontWeight: "900", color: "#15803d" }, sub: { fontSize: 12, color: "#6b7280", marginTop: 2 },
    ...
    statDot: { width: 8, height: 8, borderRadius: 4 }, statCount: { fontSize: 22, fontWeight: "900" }, statLabel: { fontSize: 11, fontWeight: "700" },
    list: { padding: 16, gap: 10 },
    emptyBox: { alignItems: "center", paddingVertical: 60 }, emptyText: { color: "#9ca3af", fontWeight: "600", fontSize: 15 },
    ...
    cardInfo: { flex: 1 }, cardName: { fontSize: 15, fontWeight: "700", color: "#111827" }, cardEmail: { fontSize: 12, color: "#6b7280", marginTop: 1 },
    statusBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
    statusDot: { width: 6, height: 6, borderRadius: 3 }, statusText: { fontSize: 11, fontWeight: "700" },
    ...
    mName: { fontSize: 20, fontWeight: "800", color: "#111827" }, mEmail: { fontSize: 13, color: "#6b7280", marginTop: 2 },
    closeBtn: { padding: 8, backgroundColor: "#f3f4f6", borderRadius: 12 },
    ...
    infoLbl: { fontSize: 11, color: "#9ca3af", fontWeight: "600", textTransform: "uppercase" }, infoVal: { fontSize: 15, fontWeight: "700", color: "#111827", marginTop: 2 },
